...
def run(playwright: Playwright) -> None:
    ...
    # --- COUNTRY FILTER ---
    print("[INFO] Applying 'Norway' filter...")
    page.get_by_role("button", name="Filter").click()
    page.wait_for_selector("button:has-text('Clear')")
    page.wait_for_timeout(1200)
    page.get_by_role("button", name="Clear").click()
    page.wait_for_timeout(1200)
    page.get_by_role("checkbox", name="Norway Norway").check()
    page.wait_for_timeout(1200)
    ...
    ics_path = os.path.join(download_dir, f"ICS/NOR_{last_year}.ics")
    ...
    csv_path = os.path.join(download_dir, f"CSV/NOR_{last_year}.csv")
